...
(
    ...
    `no_request`               TINYINT(1)            DEFAULT 0 COMMENT '用户未提问人工（转人工后）',
    `manul_first_message_id`   VARCHAR(255)          DEFAULT NULL COMMENT '客服第一条消息ID',
    `manul_first_message_time` DATETIME              DEFAULT NULL COMMENT '客服第一条回复时间',
    `manul_last_message_id`    VARCHAR(255)          DEFAULT NULL COMMENT '客服最后一条消息ID',
    `manul_last_message_time`  DATETIME              DEFAULT NULL COMMENT '客服最后一条回复时间',
    `manul_reply_num`          INT(11)               DEFAULT 0 COMMENT '客服回复次数',
    `no_reply`                 TINYINT(1)            DEFAULT 0 COMMENT '客服未回复（人工会话）',
    PRIMARY KEY (`aid`),
